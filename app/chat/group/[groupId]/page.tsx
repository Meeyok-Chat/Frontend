...
export default function GroupChat() {
  ...
  useEffect(() => {
    setGroup(
      mockGroups[groupId as keyof typeof mockGroups] || {
        id: groupId,
        name: "Unknown Group",
        avatar: "/placeholder.svg?height=40&width=40",
    ...
    const mockMessages = [
      ...
      },
      {
        ...
        senderId: "me",
        senderName: "You",
        text: "Glad to have everyone here.",
    ...
    <div className="flex flex-col h-[calc(100vh-8rem)]">
      <Card className="flex flex-col h-full">
        <CardHeader className="flex flex-row items-center gap-4 pb-4 border-b">
          ...
          <Button variant="ghost" size="icon" onClick={() => setShowMembers(!showMembers)} aria-label="Show members">
        ...
        <div className="flex flex-1 overflow-hidden">
          <CardContent className="flex-1 overflow-y-auto p-4 space-y-4">
            {messages.map((message) => (
              <div key={message.id} className={`flex ${message.senderId === "me" ? "justify-end" : "justify-start"}`}>
                ...
                >
                  {message.senderId !== "me" && <p className="text-xs font-medium mb-1">{message.senderName}</p>}
                  <p>{message.text}</p>
          ...
          {showMembers && (
            <Drawer open={showMembers} onOpenChange={setShowMembers}>
              <DrawerContent>
                <DrawerHeader>
                  <DrawerTitle>Group Members ({group.members.length})</DrawerTitle>
                  <DrawerDescription>People in this conversation</DrawerDescription>
                </DrawerHeader>
                    ...
                    {group.members.map((member: any) => (
                      <li key={member.id} className="flex items-center gap-3">
                        ...
                        <div>
                          ...
                          <p className="text-xs text-slate-500">{member.id === "me" ? "You" : "Online"}</p>
                        </div>
                      </li>
                    ))}
                  </ul>
            ...
            <Drawer open={showAddMembers} onOpenChange={setShowAddMembers}>
              <DrawerContent>
                <DrawerHeader>
                  ...
                  <DrawerDescription>Add new people to this group</DrawerDescription>
                </DrawerHeader>
                <div className="px-4 py-2">
                  <Input placeholder="Search users..." className="mb-4" />
                  <ul className="space-y-4">
                    {[
                      { id: "user4", name: "Jordan Lee", avatar: "/placeholder.svg?height=40&width=40" },
                      { id: "user5", name: "Casey Brown", avatar: "/placeholder.svg?height=40&width=40" },
                      { id: "user6", name: "Riley Green", avatar: "/placeholder.svg?height=40&width=40" },
                    ].map((user) => (
