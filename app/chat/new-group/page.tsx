          ...
          <div className="border rounded-md">
            <div className="max-h-60 overflow-y-auto p-1">
              {filteredUsers.map((user) => (
                <div key={user.id} className="flex items-center space-x-2 p-2 hover:bg-slate-50 rounded-md">
                  <Checkbox
                    id={`user-${user.id}`}
                    checked={selectedUsers.includes(user.id)}
                    onCheckedChange={() => toggleUserSelection(user.id)}
                  />
                  <Label htmlFor={`user-${user.id}`} className="flex items-center space-x-2 flex-1 cursor-pointer">
                    <div className="relative">
              ...
              {filteredUsers.length === 0 && <div className="p-4 text-center text-slate-500">No users found</div>}
            </div>
          </div>
